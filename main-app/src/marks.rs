...
impl fmt::Display for Student {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "(Name: {}, Marks: {}, Class: {})",
            self.name, self.marks, self.class
        )
    }
}
...
pub fn process_marks_sheet(filename: &str) -> Vec<Student> {
    let contents: Vec<Result<Student, <Student as FromStr>::Err>> = fs::read_to_string(filename)
    ...
    for stud in contents {
        match stud {
            Ok(mut s) => {
                //mutable can be done on a non-mutable top level element
